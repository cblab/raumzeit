...
from causal_set_engine.policies.phase2_gate import Phase2GateDecision, Phase2GateInput, evaluate_phase2_gate
...
def evaluate_age_biased_phase2c_scan(
    *,
    n_values: list[int],
    runs: int = 8,
    seed_start: int = 100,
    ...
    return Phase2CScanResult(gate_decision=gate, settings=settings, rows=tuple(rows))
